...
interface CategoryEditorProps {
  ...
  onCategoriesChange: (categories: CategoryStructure) => void;
}

const CategoryEditor = ({ categories, onCategoriesChange }: CategoryEditorProps) => {
  const { user } = useAuth()
      ...
      <Box bg={boxBg} p={6} rounded="lg" shadow="md" borderWidth="1px" borderColor={borderColor}>
        <VStack spacing={6} align="stretch">
          <Flex justifyContent="space-between" alignItems="center">
            <Text fontSize="lg" fontWeight="bold" color={headerColor}>Categories</Text>
            <Button 
              colorScheme="blue" 
              size="sm" 
